...
function bps_get_server_ip_address_sysinfo() {
	...
	if ( is_admin() && wp_script_is( 'bps-accordion', $list = 'queue' ) && current_user_can('manage_options') ) {
		if ( isset( $_SERVER['SERVER_ADDR'] ) ) {
			...
			echo '<strong><span class="sysinfo-label-text">'.__('Server|Website IP Address: ', 'bulletproof-security').'</font></strong>'.$ip.'<br>';		
		} else { 
			$ip = @dns_get_record( bpsGetDomainRoot(), DNS_ALL );
			echo '<strong><span class="sysinfo-label-text">'.__('Server|Website IP Address: ', 'bulletproof-security').'</font></strong>'.$ip[0]['ip'].'<br>';	
		}
	}
}
...
<?php 
	...
	if ( is_callable('xml_parser_create') ) { 
		$text = __('Yes', 'bulletproof-security').'</font>';
		echo $text.'<br>';
	} else { 
		$text = __('No', 'bulletproof-security').'</font>';
		echo $text.'<br>';
	}
