...
$bpsSuccessMessageDef = '<font color="green"><strong>'.__('Success! Your Default Mode Master htaccess file was created successfully!', 'bulletproof-security').'</strong></font><br><font color="#fb0101"><strong>'.__('CAUTION: Default Mode should only be activated for testing or troubleshooting purposes. Default Mode does not protect your website with any security protection.', 'bulletproof-security').'</strong></font><br><font color="black"><strong>'.__('To activate Default Mode for troubleshooting, select the Default Mode radio button and click the Activate button to put your website in Default Mode.', 'bulletproof-security').'</strong></font>';
...
if ( isset( $_POST['bps-auto-write-default'] ) && current_user_can('manage_options')) {
	...
	if ( $HFiles_options['bps_htaccess_files'] == 'disabled' ) {
		echo $bps_topDiv;
		$text = '<font color="blue"><strong>'.__('htaccess Files Disabled: default.htaccess (Default Mode) Master file writing is disabled.', 'bulletproof-security').'</strong></font><br>';			
		echo $text;
    	echo $bps_bottomDiv;
		return;
	}
...
if ( isset( $_POST['bps-auto-write-secure-root'] ) && current_user_can('manage_options')) {
	...
	if ( $HFiles_options['bps_htaccess_files'] == 'disabled' ) {
		echo $bps_topDiv;
		$text = '<font color="blue"><strong>'.__('htaccess Files Disabled: secure.htaccess Root Folder BulletProof Mode Master file writing is disabled.', 'bulletproof-security').'</strong></font><br>';			
		echo $text;
    	echo $bps_bottomDiv;
		return;
	}
