...
;(function(window, document) {
  "use strict";
  var pluginName = 'shocking_particleground';
    ...
    Particle.prototype.draw = function() {
      ...
      for (var i = particles.length - 1; i > this.stackPos; i--) {
        ...
        if (dist < options.proximity) {
          ...
          if (options.curvedLines) {
            ...
            var alpha = (1 - (dist / options.proximity));
            ctx.strokeStyle = 'rgba(' + getR(options.lineColor) + ', ' + getG(options.lineColor) + ', ' + getB(options.lineColor) + ', ' + alpha + ')';
            
            ctx.beginPath();
              ctx.moveTo(this.position.x + this.parallaxOffsetX, this.position.y + this.parallaxOffsetY); // start from here
            
              // wiggly woggle wob shock!
              for(var currentPoint = 0.25; currentPoint < 1; currentPoint += 0.20) {
                var ptx = ((this.position.x + this.parallaxOffsetX) + ((p2.position.x + p2.parallaxOffsetX) - (this.position.x + this.parallaxOffsetX)) * currentPoint),
                    pty = ((this.position.y + this.parallaxOffsetY) + ((p2.position.y + p2.parallaxOffsetY) - (this.position.y + this.parallaxOffsetY)) * currentPoint);

                ctx.lineTo(ptx + 12 - (Math.random() * 20), pty + 12 - (Math.random() * 20));
              }
              
              // end here!
              ctx.lineTo(p2.position.x + p2.parallaxOffsetX, p2.position.y + p2.parallaxOffsetY);
            // ctx.stroke(); // stroke before closePath, prevent a line back from last point
            ctx.closePath();
            ctx.stroke(); // stroke after closePath, render a line back from last point
          }
        }
      }
      ...
      if (options.grabHover) {
        ...
        if (dist < options.grabDistance) {
          ...
          var alpha = (1 - (dist / options.grabDistance));
            ctx.strokeStyle = 'rgba(' + getR(options.lineColor) + ', ' + getG(options.lineColor) + ', ' + getB(options.lineColor) + ', ' + alpha + ')';
            ...
            ctx.beginPath();
              ...
              // wiggly woggle wob shock!
              for(var currentPoint = 0.25; currentPoint < 1; currentPoint += 0.20) {
                var ptx = ((this.position.x + this.parallaxOffsetX) + ((pointerX - offsetLeft) - (this.position.x + this.parallaxOffsetX)) * currentPoint),
                    pty = ((this.position.y + this.parallaxOffsetY) + ((pointerY - offsetTop) - (this.position.y + this.parallaxOffsetY)) * currentPoint);
