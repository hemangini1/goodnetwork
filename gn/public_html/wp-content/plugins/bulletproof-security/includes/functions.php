	...
	if ( current_user_can('manage_options') ) {
		...
		if ( esc_html($_SERVER['QUERY_STRING']) == 'page=bulletproof-security/admin/system-info/system-info.php' ) {
			...
			if ( 500 == $status_code8 && 500 == $status_code10 || 500 == $status_code9 ) {
				...
				if ( 301 == $status_code9 || 302 == $status_code9 || 200 == $status_code9 ) {
					...
					$text = '<font color="#2ea2cc"><strong>'.$status_code9.':</strong></font> '.__('mod_rewrite Inconclusive: Status is not 200, 301 or 302', 'bulletproof-security').'<br>';
					echo $text;				
				}
			} // End: Fubar condition
